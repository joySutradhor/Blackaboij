...
const Accessories = () => {
    ...
    useEffect(() => {
        ...
        fetch(accessoriesCollectionsProducts)
            ...
            .then(data => {
                setAccessoriesCollections(data);
                console.log(data);
            })
            .catch(error => {
            ...
            <div className="grid md:grid-cols-3 grid-cols-2 gap-[10px] md:gap-[25px] mt-[50px]  md:mx-[50px] mx-[20px] ">
                {accessoriesCollections.map((product) => (
                    <div key={product.id} className={`bg-[#B7B7B7] product-card font-custom ${isHovered === product.id && !isScrolling ? 'fade' : ''}`} onMouseEnter={() => handleMouseEnter(product.id)} onMouseLeave={handleMouseLeave}>
                        <Link to={`productaccessories/${product.id}`}> <img src={product.img} alt={product.productName} className="front-img" /></Link>
                        <Link to={`productaccessories/${product.id}`}> <img src={product.backImg} alt="" className="back-img " /></Link>
